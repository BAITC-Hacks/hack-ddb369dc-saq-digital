...
Статус: реализован в Backend; изменения интеграции описаны ниже.
...
существующий `POST /api/cart` с новым `confirmationId` и
`expectedUnitPriceKzt: candidate.product.priceKzt` из показанной карточки.
Поле цены опционально для совместимости, но необходимо для точной привязки
подтверждения к цене, в том числе при нескольких вкладках. Массового добавления нет.
В live-режиме корзина повторно проверяет карточку у партнёра. При
`PRODUCT_PRICE_CHANGED` (409) повторите поиск, покажите новую цену и запросите
подтверждение; при `PRODUCT_VERIFICATION_UNAVAILABLE` (503) предложите повтор
позже. Корзина при этих ошибках не меняется. Подробности: [контракт корзины](backend-api.md#session-and-cart).
...
| 400 | `PAYMENT_DATA_NOT_ALLOWED` | В имени файла обнаружены платёжные реквизиты |
...
делит лимит OpenAI-запросов с чатом: 60 попыток за скользящие 60 секунд и до
4 одновременных вызовов провайдера (`openai.maxCalls`, `windowMs`, `maxConcurrent`).
Ограничение временное, не требует перезапуска сервера. При `AI_CALL_LIMIT`
объект `error` может дополнительно содержать `retryAfterSeconds`; после указанной
задержки создайте новую попытку с новым `requestId`.
...
Не загружайте файлы с номерами карт/IBAN, CVV/CVC, PIN, кодами оплаты и другими
платёжными реквизитами. Имя файла проверяется до создания задания; обнаруженные
значения в извлечённых строках и ответах модели маскируются. Проверки используют
шаблоны и контрольные суммы и не охватывают все способы записи. Они не удаляют
реквизиты из исходного бинарного файла до отправки провайдеру.

Источник ограничений обработки: [OpenAI File inputs](https://developers.openai.com/api/docs/guides/file-inputs).

## Проверки реализации

Последние изменения проверки цены корзины, фильтра платёжных данных и
возобновляемой квоты AI **не проверены** тестами или сборкой; запуск ожидает
отдельного согласования. Описанные ниже результаты относятся к прежней реализации.
