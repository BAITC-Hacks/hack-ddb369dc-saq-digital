...
Import runs in the background and may take several minutes. `GET /api/health` stays available with HTTP 200 as a liveness check and returns an optional `catalog` object: `status` (`loading`, `ready`, `failed`), `source` (`partner`, `local`), `products`, and progress fields `pages`, `listed`, `completed`, `failed`; `loadedAt` is the completed snapshot timestamp. HTTP 200 does **not** imply the catalog is ready. While loading, other `/api/*` routes return HTTP 503, `Retry-After: 10` and `{ "error": { "code": "CATALOG_LOADING", "message": "..." } }`. A failed import returns HTTP 503 with `CATALOG_UNAVAILABLE`. New sessions are accepted only after the imported catalog is published. Refreshing prices/stock requires restarting the backend. This is a read-only partner integration; the cart does not reserve partner stock.
