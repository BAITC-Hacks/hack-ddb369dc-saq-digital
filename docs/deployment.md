...
Неактивные сессии истекают через 30 минут, максимум одновременно — 1000. После
достижения лимита новая сессия получает 503, а активные корзины сохраняются.
...
В чате видны ошибки соединения, локальный режим и временное исчерпание лимита.
По умолчанию допускается 20 вызовов OpenAI за 60 секунд на Backend и 10 за минуту
на IP; одна сессия получает не более 6. Анонимному источнику доступны 2 вызова
за минуту. Параметры, 768 выходных токенов и таймаут 15 секунд находятся в
`backend/config.json`. Backend доверяет `X-Forwarded-For` только от адреса
Docker-сервиса `frontend`, указанного в `trustedProxyHost`. Адрес обновляется
через DNS каждые 30 секунд; при ошибке DNS доверие отключается. Если перед Nginx
есть ещё один reverse proxy, настройте его цепочку доверия и лимиты под нагрузку.
Прямому клиенту с произвольным `X-Forwarded-For` доверять нельзя. Ключи и полный
`.env` публиковать не нужно.
