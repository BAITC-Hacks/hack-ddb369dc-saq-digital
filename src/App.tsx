...
function Widget({ onCartChanged, language, onLanguageChange, readingMode, onReadingModeChange, conversation, setConversation, chats, chatId, query, setQuery, onSelectChat, onNewChat, onDeleteChats, storageError, historyConflict }: ReadingModeProps & {
  ...
  const changeChat = (id?: string) => {
    if (loading || searchInFlight.current || confirmationInFlight.current || confirmationVisible) return
    setError(null)
    setDeleteTarget(null)
    historyReturnFocus.current = 'composer'
    followNewest.current = true
    setHistoryOpen(false)
    const reuseActive = !id && conversation.length === 0 && !query.trim()
  ...
  const navigationBusy = loading || confirmLoading || confirmationVisible
  ...
  return <>
    {open && <aside ref={widgetRef} className={`widget${readingMode ? ' reading-mode' : ''}`} aria-labelledby="ekt-chat-title" role="dialog" aria-modal="false" lang={language ?? 'ru'}>
      ...
      {historyOpen ? <section id="ekt-chat-history" className="chat-history" aria-labelledby="ekt-history-title">
        ...
        <ul className="chat-list" aria-label={t.dialogues}>{chats.map((chat) => {
          ...
          const title = firstTurn?.query ?? (chat.draft.trim() || t.untitledChat)
          ...
          {conversation.map((turn) => {
            ...
            return <div className="conversation-turn" key={turn.id}>
              ...
              {result && <>
                ...
                {!result.exactMatch && result.alternatives.length === 0 && result.intent !== 'purchase_terms' && <div className="empty-result"><Package size={23} aria-hidden="true" /> {t.noResults}</div>}
              </>}
    ...
    {selected && confirmationVisible && <div className="shade" role="presentation"><section ref={confirmationRef} className={`confirm${readingMode ? ' reading-mode' : ''}`} role="dialog" aria-modal="true" aria-labelledby="ekt-confirm-title" aria-describedby={confirmAttempted ? 'ekt-confirm-status' : undefined} tabIndex={-1} onKeyDown={keepConfirmationFocus} lang={language ?? 'ru'}>
      ...
      <footer><button className="cancel" type="button" onClick={closeConfirmation}>{confirmAttempted ? t.hideRequest : t.cancel}</button><button ref={confirmButtonRef} className="yes" type="button" disabled={confirmLoading || Boolean(quantityIssue)} onClick={() => void confirm()}>{confirmLoading ? t.adding : confirmAttempted ? t.retry : t.yesAdd}</button></footer>
    </section></div>}
  </>
}
...
function App() {
  ...
  return <main className="store">
    ...
    {onCartRoute ? <CartScreen cart={cart} error={cartError} language={language} onBack={backToCatalog} {...readingProps} /> : <>
      ...
      <Widget onCartChanged={openCart} language={language} onLanguageChange={changeLanguage} conversation={activeChat.turns} setConversation={setConversation} chats={chats} chatId={activeChat.id} query={activeChat.draft} setQuery={setDraft} onSelectChat={setSelectedChatId} onNewChat={newChat} onDeleteChats={deleteChats} storageError={storageError} historyConflict={historyConflict} {...readingProps} />
    </>}
  </main>
}
