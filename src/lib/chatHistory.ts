...
function product(value: unknown): ApiProduct {
  ...
  return {
    ...
    stock: number(item.stock, 0, true), priceKzt: number(item.priceKzt),
    ...
    ...(item.stores === undefined ? {} : { stores: list(item.stores, 100).map((value) => {
      ...
      return { id: number(store.id, Number.MIN_SAFE_INTEGER, true), name: text(store.name, 1000), quantity: number(store.quantity, 0, true) }
    }) }),
...
function response(value: unknown): SearchResponse {
  ...
  return {
    intent: choice(item.intent, ['product', 'specifications', 'purchase_terms'] as const),
...
function error(value: unknown): UiError {
  const item = record(value)
  return {
    key: choice(item.key, ['shortQuery', 'requestFailed', 'requestTimeout', 'serverUnavailable'] as const),
