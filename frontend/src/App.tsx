...
import { addToCart, ApiError, frontendCartUrl, getCart, pendingConfirmationId, searchCatalog } from './lib/api'
...
function Widget({ onCartChanged, language, onLanguageChange, readingMode, onReadingModeChange, conversation, setConversation, chats, chatId, query, setQuery, onSelectChat, onNewChat, onDeleteChats, storageError, historyConflict, upload, setUpload, uploadNames }: ReadingModeProps & {
  ...
  const submit = async (value = query, retryTurn?: ChatTurn) => {
    ...
    if (retryTurn && (retryTurn.id !== latestTurn?.id || retryTurn.restored || retryableQuestions[chatId] !== retryTurn.id)) return
    ...
    setError(null)
    setConversation((turns) => [...turns.filter((turn) => turn.id !== retryTurn?.id), { id, query: message, language: messageLanguage, status: 'loading' }])
    try {
      const response = await searchCatalog(backendSearchQuery(message, messageLanguage), chatId)
      if (response.notice && ['AI_UNAVAILABLE', 'AI_CALL_LIMIT', 'AI_RATE_LIMIT'].includes(response.notice)) {
      ...
      if (!(caught instanceof ApiError) || caught.status === 0 || caught.status === 429 || caught.status >= 500) {
        setRetryableQuestions((questions) => ({ ...questions, [chatId]: id }))
      }
  ...
  const choose = (product: ApiProduct, quantity: number, trigger: HTMLButtonElement, sourceExpiresAt?: number) => {
    ...
    const retryId = retryConfirmations.current[JSON.stringify([product.sku, quantity])] ?? pendingConfirmationId(product.sku, quantity)
        ...
        <div role="log" aria-label={t.conversationHistory} aria-live="polite" aria-relevant="additions text" aria-atomic="false">
          {conversation.map((turn) => {
            ...
            return <div className="conversation-turn" key={turn.id}>
              ...
              {!historical && retryableQuestions[chatId] === turn.id && <button className="question-retry" type="button" disabled={loading || confirmLoading || confirmationVisible} onClick={() => void submit(turn.query, turn)}>{t.retryQuestion}</button>}
            </div>
          })}
        </div>
