...
async function openConfirmation(buttonName = 'Выбрать') {
  render(<App />)
  fireEvent.change(screen.getByRole('textbox', { name: 'Сообщение помощнику EKT' }), { target: { value: translations.ru.demoQuery } })
...
describe('quantity selection before cart confirmation', () => {
  ...
  it('provides the editable quantity and confirmation action in Kazakh', async () => {
    ...
    fireEvent.change(screen.getByRole('textbox', { name: 'EKT көмекшісіне хабарлама' }), { target: { value: translations.kk.demoQuery } })
