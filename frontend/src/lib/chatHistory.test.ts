...
describe('chat history storage', () => {
  ...
  it('roundtrips conversation notices and top-level quantities with fractional stock', () => {
    const original = chats()[0]
    const turn = original.turns[0]
    const result = turn.response!
    const updated: Chat[] = [{ ...original, turns: [
      { ...turn, response: { ...result, quantity: 3, exactMatch: { ...result.exactMatch!, product: { ...result.exactMatch!.product, stock: 12.5, stores: [{ id: 1, name: 'Алматы', quantity: 12.5 }] } } } },
      { id: 'conversation', query: 'да', language: 'ru', status: 'complete', response: { intent: 'conversation', answer: 'Продолжим.', quantity: 0, notice: 'AI_OFFLINE', filters: null, exactMatch: null, alternatives: [] } },
    ] }]
    expect(saveChatHistory(updated, 'chat-a')).toBe(true)
    const restored = loadChatHistory()
    expect(restored.storageError).toBe(false)
    expect(restored.chats[0].turns[0].response).toEqual(updated[0].turns[0].response)
    expect(restored.chats[0].turns[1].response).toEqual(updated[0].turns[1].response)
  })
  ...
  it.each([
    ...
    [{ id: 1, name: 'Астана', quantity: -1 }], [{ id: 1, name: 'Астана', quantity: Infinity }],
